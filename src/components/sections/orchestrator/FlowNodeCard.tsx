...
/* Category visual config — subtle accent tints, NOT loud full colors */
const CATEGORY_CONFIG: Record<
  NodeCategory,
  { label: string; accent: string; icon: (size: number) => React.ReactElement }
> = {
  agentic: {
    label: "AGENTIC",
    accent: "#36b595", // green
...
export default function FlowNodeCard({
  ...
  return (
    <div
      className={`rounded-xl overflow-hidden bg-white border border-boost-border/70 shadow-[0_1px_2px_rgba(35,21,40,0.04)] transition-all ${className}`}
    >
      {/* Left accent strip + card content */}
      <div className="relative">
        ...
        {/* Header — category label with accent-tinted icon */}
        <div className="flex items-center gap-1.5 px-3 pt-2.5 pb-1 pl-[14px]">
        ...
        <div className="px-3 pb-3 pl-[14px]">
          <p className="text-[13px] font-semibold text-boost-dark leading-snug">{displayName}</p>
          {displayDesc && (
            <p className="text-[11px] text-boost-muted mt-1 leading-relaxed line-clamp-3">
              {displayDesc}
            </p>
          )}
        </div>
      ...
      {visibleItems.length > 0 && (
        <div className="px-3 pb-2.5 pt-2 space-y-1.5 bg-boost-surface/40 border-t border-boost-border/50">
          {visibleItems.map((item, i) => (
            <div key={i} className="flex items-center gap-2">
              <svg width="11" height="11" viewBox="0 0 384 512" fill="#7a6b80" className="flex-shrink-0">
      ...
      {node?.customCardJson && (
        <div className="px-3 pb-3 pt-2 border-t border-boost-border/50 bg-boost-surface/40">
          <div className="rounded-lg border border-boost-border bg-white overflow-hidden shadow-sm">
